...
// Description : Client2 in C++ Ansi-style
//============================================================================
...
void readFromFile()
{
	char * inputFilePath = "../files/client2.txt";
	string inputKey;
	string inputValue;
	ifstream inputFile;
...
int processUserInput()
{
  ...
  for (i = 0; i < 12; i++)
	...
	continue;
      cout << "The Client 2 has received a request with search word " << inputString<< ", which maps to key " << value[i]<< " .\n";
...
int manageUDPClient1Socket()
{
    ...
    if (p == NULL) {
      cerr<<  "Client2 :: failed to bind socket\n";
        return 2;
    }
...
if ((numbytes = sendto(sockfd, key, strlen(key), 0,
 ...
 cout << "The Client 2 sends the request " << key << " to the Server 1 with port number " <<remotePort << " and IP address " <<remoteIP << " . \n";
 cout<<"The Client2's port number is " << ntohs(sin.sin_port) <<" and the IP address is " << selfIP<< ".\n";
...
(bufCopy," ");
   ...
   cout << "The Client 2 received the value " << data<< " from the Server 1 with port number " << remotePort<< "  and IP address " <<remoteIP << " . \n";
   cout << "The Client2's port number is " << ntohs(sin.sin_port)<< " and IP address is " << selfIP<< " .\n";
